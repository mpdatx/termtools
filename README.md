...
| `local_domains` | `{}`                     | WezTerm domain names whose filesystem the GUI can probe locally — used by the action picker to decide whether file-existence checks (e.g. dim `Open TODO.md` when missing) are meaningful. The built-in `'local'` domain and every entry in `config.unix_domains` are auto-included; this list is for unusual cases like a TLS client that happens to point at a mux on the same machine. |
